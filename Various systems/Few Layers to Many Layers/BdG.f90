program BDGFEWTOMANY
    implicit none
    integer :: NUMKX, NUMKY, NUMKZ, NUMK, NUMT, io, i, j, IRLATT, IRLATTMAX, kcounter, LWORK, INFO, NCELLS, ini, fin, reps, &
    & maxreps, uniquecounter, NUMCHEMTYPES, intnumdenpointer, NUME, metalorno, JATOM, rcheck,&
    ...
    &TOTDOSATMU, tempvala, tempvalb, metalepsilon, scepsilon
    ...
    if (newlayers == 0) then
        ...
        do while ((MAXVAL(diffN) > metalepsilon .or. diffchem > metalepsilon) .and. reps < maxreps)
            ...
            newchempot = chempot + 0.1*atan(diffchem) ! New chempot for next run
