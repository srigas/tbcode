program TB
	...
	&chempot, mixfactorD, readcharge, gap
	...
	& maxreps, uniquecounter, progreps, maxprogreps
    ...
    do i = 1, uniquecounter-1
        if (SORTEDEIGVALS(i)*SORTEDEIGVALS(i+1) < 0) then
            gap = SORTEDEIGVALS(i+1) - SORTEDEIGVALS(i)
        endif
    end do
    ...
    do i = 1, NUMT
        USUPCOND(i) = USUPCOND(i) + 0.001
    end do

    deallocate(UNIQUEEIGVALS)
    deallocate(SORTEDEIGVALS)
    
    if (progreps < maxprogreps) then
        goto 132
    endif
