program TB
	...
	open (1, file = 'basisvectors.dat', action = 'read')
	do i = 1,NUMT
		read(1,*) TPTS(1:3,i), E0(i), INUMEL(i), ULCN(i), rhozero(i), BETA(i)
	end do
	close(1)
    ...
    do while (MAXVAL(diff) > epsilon .and. reps < maxreps) ! Check for convergence
		...
		print *, newrho !this is done for testing purposes only

        rho = (1.0 - mixfactor)*rho + mixfactor*newrho
		reps = reps + 1

		print *, 'The values of newrho, rho are', newrho, rho
	...
	subroutine NUM_DEN(uniquecounter,SORTEDEIGVALS,EIGENVALUES,numdensity,NUMT,NUMK)
		...
		do i = 0, numenergyintervals
			...
			do j = 1, 2*NUMT*NUMK
				numdensity(2,i+1) = numdensity(2,i+1) + (2.0/pi)*delta/((numdensity(1,i+1) - EIGENVALUES(j))**2 + delta**2)
			end do
		end do
