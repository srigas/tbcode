program TB
	...
	real*8, allocatable, dimension(:,:) :: KPTS, TPTS, RLATT, RHOCHAIN, MAGNETCHAIN
    ...
    print *, 'Please enter the center atom`s B.'
	read *, BETA(INT(((NUMT+1.0)/2.0)+0.5))
	
	!BETA(INT(((NUMT+1.0)/2.0)-0.5)) = BETA(INT(((NUMT+1.0)/2.0)+0.5))
    ...
    print *, 'Please enter the center atom`s charge.'
    read *, INUMEL(INT((NUMT+1)/2))
	rhozero(INT((NUMT+1)/2)) = INUMEL(INT((NUMT+1)/2))
    ...
    ! The *2 factors are now due to spin
	allocate(EIGENVECTORS(2*NUMT,2*NUMT*NUMK))
	allocate(EIGENVALUES(2*NUMT*NUMK))
	allocate(HAMILTONIAN(2*NUMT,2*NUMT))
	allocate(TEMPEIGENVALUES(2*NUMT*NUMK))
	allocate(alphas(2*NUMT*NUMK))

	IRLATTMAX =  (2*NCELLS+1)**3

	call RPTS(a_1,a_2,a_3,NCELLS,RLATT,IRLATTMAX) !Here we construct the RLATT Matrix
	
	!The following are for the configuration of zheev. The *2 factors are due to spin
	!-------------------------------------------------
	allocate(W(2*NUMT))
	allocate(RWORK(3*(2*NUMT) - 2))
	LWORK = 2*NUMT*(2*NUMT+1)
	allocate(WORK(LWORK))
	!-------------------------------------------------
	...
	read *, epsilon
	reps = 0
	do i = 1, NUMT !This stays NUMT and not *2
		diff(i) = 1.0
	end do
		...
		do kcounter = 1, NUMK
			...
			do i = 1, NUMT 
				do j = 1, NUMT 
                    ...
                    HAMILTONIAN(i+NUMT,j) = (0.0,0.0)
                    HAMILTONIAN(i,j+NUMT) = (0.0,0.0)
                    HAMILTONIAN(i+NUMT,j+NUMT) = (0.0,0.0)
                    
					TTPRIME = TPTS(1:3,j) - TPTS(1:3,i)
					do IRLATT = 1, IRLATTMAX
                        RPOINT = RLATT(1:3,IRLATT)
						call HAM(zPauli,IdentityPauli,RPOINT + TTPRIME,NUMT,E0,j,R0,RMAX,positionhamiltonian)
						expon = exp(CI*DOT_PRODUCT(KPOINT,RPOINT))

						HAMILTONIAN(i,j) = HAMILTONIAN(i,j) + expon*positionhamiltonian(1,1)
						HAMILTONIAN(i+NUMT,j) = HAMILTONIAN(i+NUMT,j) + expon*positionhamiltonian(2,1)
						HAMILTONIAN(i,j+NUMT) = HAMILTONIAN(i,j+NUMT) + expon*positionhamiltonian(1,2)
						HAMILTONIAN(i+NUMT,j+NUMT) = HAMILTONIAN(i+NUMT,j+NUMT) + expon*positionhamiltonian(2,2)
					end do
				end do
			end do

			call zheev ('V', 'U', 2*NUMT, HAMILTONIAN, 2*NUMT, W, WORK, LWORK, RWORK, INFO) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

			ini = (kcounter-1)*2*NUMT + 1
			fin = kcounter*2*NUMT
	...
	!allocate(UNIQUEEIGVALS(2*NUMT*NUMK))
	!uniquecounter = 0
    ...
    do i = 1, NUMT
        ...
        do j = 1, 2*NUMT*NUMK
            if (alphas(j) == 1) then
                rho(i) = rho(i) + abs(EIGENVECTORS(i,j))**2 + abs(EIGENVECTORS(i+NUMT,j))**2
            endif
		end do
		rho(i) = rho(i)/(NUMK)
	end do
	...
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	allocate(MAGNETCHAIN(2,NUMT))
	do i = 1, NUMT
		MAGNETCHAIN(1,i) = i
		MAGNETCHAIN(2,i) = magnet(i)
	end do
	open(17, file = 'magnetchain.txt', action = 'write')
		do j = 1, NUMT
			write (17,101) (MAGNETCHAIN(i,j), i = 1,2)
		end do
		101 format(3F17.8)
	close(17)
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	subroutine HAM(zPauli,IdentityPauli,TTPRIME,NUMT,E0,j,R0,RMAX,positionhamiltonian)
        ...
        if (norm2(TTPRIME) == 0.0) then ! This case corresponds to t = t', R = 0
            positionhamiltonian = (E0(j) + ULCN(j)*(rho(j) - rhozero(j)))*IdentityPauli + BETA(j)*zPauli
