program TB
	implicit none
	real*8 :: a_1(3), a_2(3), a_3(3), RMAX, R0, TTPRIME(3), KPOINT(3), RPOINT(3), TEMPVAL, epsilon
	real*8, allocatable, dimension(:) :: W, RWORK, E0, ULCN, rho, newrho, rhozero, EIGENVALUES, diff
	...
	integer :: NUMKX, NUMKY, NUMKZ, NUMK, NUMT, io, i, j, IRLATT, IRLATTMAX, kcounter, LWORK, INFO, NCELLS, NUMEL, ini, fin, reps, &
	& maxreps
	...
	do while (MAXVAL(diff) > epsilon .and. reps < maxreps) !Check for convergence
		...
		!Now that we have the Fermi energy as EIGENVALUES(NUMEL) we proceed by calculating the new charges.
		do i = 1, NUMT
			newrho(i) = 0.0
			do j = 1, NUMEL !sum for E < E_F
